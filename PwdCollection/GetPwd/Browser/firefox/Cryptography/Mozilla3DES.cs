namespace Firefox.Cryptography
{
    ...
    class Mozilla3DES
    {
        ...
        public Mozilla3DES(byte[] GlobalSalt, byte[] MasterPassword, byte[] EntrySalt)
        {
        ...
        public void Compute()
        {
            ...
            int len = this.EntrySalt.Length + (20 - this.EntrySalt.Length);
            PES = new byte[len];
            Array.Copy(this.EntrySalt, 0, PES, 0, len);
            for (int i = this.EntrySalt.Length; i < len; i++)
            {
                PES[i] = 0;
            }
            //PESES
